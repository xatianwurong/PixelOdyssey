...
public:
    ...
    /**
     * @brief 获取射线上的点
     * @param t 距离参数（t=0 为起点，t=1 为方向向量长度处）
     * @return 射线上的点坐标
     */
...
/**
 * @brief 射线投射结果结构体
 * @details 存储射线与物体相交的详细信息
 */
struct RaycastHit
{
    Entity entity;       ///< 命中的实体
    glm::vec3 point;     ///< 击中点坐标
    glm::vec3 normal;    ///< 击中点法线
    float distance;      ///< 距离起点的距离
    bool hasHit;         ///< 是否击中物体
 ...
 * @brief 射线投射工具类
 * @brief 提供射线与几何体的相交检测
 * @details 提供静态方法进行各种射线投射操作
 * @details 支持 AABB、球体、平面等多种几何体
 * @功能描述 实现 3D 空间的射线碰撞检测
 * @设计目的 为编辑器提供拾取和交互能力
 * @使用场景 鼠标拾取、碰撞检测、可见性测试
 * @关键实现 使用数学公式计算射线与几何体的交点
 */
class Raycast
{
public:
    ...
    /**
     * @brief 射线与 AABB（轴对齐包围盒）相交检测
     * @param ray 射线
     * @param min AABB 最小点
     * @param max AABB 最大点
     * @return 如果相交返回 true
     * @details 使用 slab 方法检测相交
     */
