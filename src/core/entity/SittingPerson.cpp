#include "SittingPerson.h"
#include <GL/glew.h>
#include <glm/gtc/matrix_transform.hpp>

SittingPerson::SittingPerson(const glm::vec2& position, const glm::vec4& color, float size)
    : Shape(position, color, 1.0f), m_size(size)
{
    // Generate vertices
    float bodyHeight = m_size * 0.3f;
    float headSize = m_size * 0.15f;
    float armLength = m_size * 0.2f;
    float legLength = m_size * 0.2f;
    ...
    m_vertices = {
        ...
        armLength, bodyHeight * 0.3f, // Right arm bottom
        0.0f, bodyHeight * 0.7f,      // Left arm top
        -armLength, bodyHeight * 0.3f, // Left arm bottom
