...
House::House(const glm::vec2& position, const glm::vec4& color, float size)
    : Shape(position, color, 1.0f), m_size(size)
{
    // Generate vertices
    float width = m_size;
    float height = m_size;
    float roofHeight = m_size * 0.3f;
    ...
    m_vertices = {
        ...
        // Roof
        -width/2 - width*0.1f, height/2,  // Left bottom
         width/2 + width*0.1f, height/2,  // Right bottom
         0.0f, height/2 + roofHeight     // Top
    };
