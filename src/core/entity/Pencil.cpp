...
Pencil::Pencil(const glm::vec2& position, const glm::vec4& color, float size)
    : Shape(position, color, 1.0f), m_size(size)
{
    // Generate vertices
    float length = m_size;
    float width = m_size * 0.1f;
    float tipLength = m_size * 0.2f;

    m_vertices = {
        // Pencil body
        -width/2, 0.0f,              // Bottom left
         width/2, 0.0f,              // Bottom right
         width/2, length - tipLength, // Top right
        -width/2, length - tipLength, // Top left
        
        // Pencil tip
        -width/2, length - tipLength, // Bottom left
         width/2, length - tipLength, // Bottom right
         0.0f, length                 // Tip
    };
